...
float ATankAIController::GetRotationgValue()
{
	...
	//FVector upDirection = TankPawn->GetActorUpVector();
	...
	//float upAngle =
	//	FMath::RadiansToDegrees(acosf(FVector::DotProduct(upDirection, moveDirection)));
	...
	//	TEXT("AI Rotation forwardAngle: %f rightAngle: %f rotationValue:% f"), 
	//	forwardAngle, rightAngle, rotationValue);
...
bool ATankAIController::IsPlayerSeen()
{
	...
	//FVector start = TankPawn->GetEyesPosition();
	//FVector end = TankPawn->GetTurretForwardVector() * 1000 + start;
		...
		ECollisionChannel::ECC_Visibility, traceParams))
	{
		...
		if (actor)
		{
			DrawDebugLine(GetWorld(), eyesPos, hitResult.Location, FColor::Cyan,
				false, 0.5f, 0, 10);
			return true; //actor == PlayerPawn;
		}
	}
	//DrawDebugLine(GetWorld(), eyesPos, playerPos, FColor::Cyan, 
	//	false, 0.5f, 0, 10);
